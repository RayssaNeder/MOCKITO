...
class GeradorDePagamentoTest {
	...
	@Test
	void deveriaCriarPagamentoParaVencedorInicioOuMeioDaSemana() {
		...
		Assert.assertEquals(date.plusDays(1), pagamento.getVencimento());
	...
	@Test
	void deveriaCriarPagamentoParaVencedorSabado() {
		Leilao leilao =  leilao();
		Lance vencedor = leilao.getLanceVencedor();
		...
		Assert.assertEquals(date.plusDays(2), pagamento.getVencimento());
		Assert.assertEquals(vencedor.getValor(), pagamento.getValor());
		Assert.assertFalse(pagamento.getPago());
		Assert.assertEquals(vencedor.getUsuario(), pagamento.getUsuario());
		Assert.assertEquals(leilao, pagamento.getLeilao());
	...
	@Test
	void deveriaCriarPagamentoParaVencedorDomingo() {
		Leilao leilao =  leilao();
		Lance vencedor = leilao.getLanceVencedor();
	...
	private Leilao leilao(){
		 Leilao leilao = new Leilao("Calular", new BigDecimal("500"), new Usuario("Fulano"));
